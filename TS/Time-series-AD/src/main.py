...
from utils.utils import version_build, save_directory, load_resume_model, Float32Encoder
...
def main(cfg):
    ...
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        log_metrics(cfg.MODEL.modelname, cfg.DATASET.dataname, test_metrics, _logger)

        if cfg.TRAIN.wandb.use:
            wandb_test_log = {f"test_{k}": v for k, v in test_metrics.items()}
            wandb.log(wandb_test_log)
            
            for k, v in test_metrics.items():
                wandb.run.summary[f"final_test/{k}"] = v

        json.dump(test_metrics, open(os.path.join(savedir, 
