...
def training_dl(
    ...
    for epoch in range(epochs):
        ...
        for idx, item in enumerate(trainloader):
            ...
            batch_x, batch_y, batch_x_mark, batch_y_mark = item
            batch_x = batch_x.float() # (batch_size, seq_len, var)
            batch_y = batch_y.float() # (batch_size, label_len + pred_len, var)
            batch_x_mark = batch_x_mark.float() # (batch_size, seq_len, time_feature_dim)
            batch_y_mark = batch_y_mark.float() # (batch_size, label_len + pred_len, time_feature_dim)

            input_ts = batch_x

            label_y = batch_x[:, -label_len:, :]
            label_mark = batch_x_mark[:, -label_len:, :]

            dec_zeros = torch.zeros([batch_y.shape[0], pred_len, batch_y.shape[-1]]).float().to(accelerator.device)
            dec_inp = torch.cat([label_y, dec_zeros], dim=1).float()
            dec_mark = torch.cat([label_mark, batch_y_mark], dim=1).float()

            outputs = model(batch_x, batch_x_mark, dec_inp, dec_mark)

            target_ts = batch_y[:, -pred_len:, :]
            ...
            global_step += 1
            if use_wandb and (wandb_iteration+1) % wandb_iter and accelerator.is_main_process:
            ...
            if global_step % wandb_iter == 0 and accelerator.is_main_process:
                train_results = OrderedDict([
                    ('lr',optimizer.param_groups[0]['lr']),
                    ('train_loss',losses_m.avg),
                    ('epoch', epoch+1)
        ...
        if (epoch+1) % eval_epochs == 0:
            ...
            if use_wandb and accelerator.is_main_process:
                ...
                wandb.log({
                    "val_loss": eval_metrics[ckp_metric],
                    "epoch": epoch+1
                }, step=global_step)
...
def test_dl(model, dataloader, criterion, accelerator: Accelerator, 
    ...
    with torch.no_grad():
        for idx, item in enumerate(dataloader):
            ...
            dec_inp = torch.zeros([batch_y.shape[0], pred_len, batch_y.shape[-1]]).float().to(accelerator.device)
            dec_inp = torch.cat([batch_y[:,:label_len,:], dec_inp], dim=1).float().to(accelerator.device)
